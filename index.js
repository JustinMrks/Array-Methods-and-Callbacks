...
console.log(getWinner(getFinals));
...
getWinnersByYear(getYears(getFinals), getWinner(getFinals));

/* Task 6: Write a function called `getAverageGoals` that accepts a parameter `data` and returns the the average number of home team goals and away team goals scored per match (Hint: use .reduce and do this in 2 steps) */



function getAvgGoals(data){
    const home = [];
    const away = [];
    data.forEach((item) => {
    ...
    let avgHome = home.reduce((item,accumulator) => item+accumulator,0);
    let avgAway = away.reduce((item,accumulator) => item+accumulator,0);
    avgHome /= home.length;
    avgAway /= away.length;
    return {avgHome, avgAway};
}
console.log(getAvgGoals(fifaData))
